...
async def test_exact_editor_heading_uses_guarded_fast_print() -> None:
    """Short exact headings must not fall back to layout-sensitive HID."""

    backend = FakeBackend()
    backend.guarded_exact_print = True  # type: ignore[attr-defined]
    heading = "# Release 1.0"
    assert len(heading) < typing_module.FAST_EDITOR_PRINT_MIN
    orig_print = backend.print_text

    async def printing(text: str) -> None:
        await orig_print(text)
        backend.set_screen(heading)
...
async def test_exact_unverified_spacing_uses_one_bounded_settled_reread() -> None:
    """Repeated exact glyph reads cannot replace calibrated spacing proof."""

    intended = "## Fixed"

    class ExactGlyphUncalibratedOCR:
        def __init__(self) -> None:
            self.calls = 0
...
async def test_short_field_prefers_compact_text_above_larger_control_effect(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Run-like fields must not localize to the enabled button below them."""
    ...
    class RunLikeBackend(FakeBackend):
        def __init__(self) -> None:
            ...
            self.native_regions: list[Region] = []
    ...
    assert result.field_text == intended
    assert result.emitted_exactly_once is True
    assert backend.native_regions
    assert backend.native_regions[0].y < 630
    _assert_no_enter(backend)
