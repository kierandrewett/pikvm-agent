...
def test_ocr_provider_selection_and_fallback() -> None:
    ...
    # paddleocr requested but the extra isn't installed -> must NOT be Paddle
    fallback = build_ocr_provider(AppConfig(ocr=OcrConfig(provider="paddleocr")), be)
    assert fallback.__class__.__name__ != "PaddleOCRProvider"
