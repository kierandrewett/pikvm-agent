...
import { RunActivity } from "@/components/workspace/run-activity";
...
export type ThreadProps = {
  ...
  readOnly?: boolean;
  activity?: RunSnapshot["active_activity"];
  working?: boolean;
};
...
const ThreadActivityContext = createContext<{
  activity?: RunSnapshot["active_activity"];
  working: boolean;
}>({ working: false });
...
export const Thread: FC<ThreadProps> = ({
  ...
  return (
    <ThreadComponentsContext.Provider value={components}>
      <ThreadActivityContext.Provider value={{ activity, working }}>
        <ThreadRoot isEmpty={isEmpty} readOnly={readOnly} />
      </ThreadActivityContext.Provider>
    </ThreadComponentsContext.Provider>
  );
};

const ThreadRoot: FC<{ isEmpty: boolean; readOnly: boolean }> = ({
  isEmpty,
  readOnly,
}) => {
...
const AssistantMessage: FC = () => {
  ...
  return (
    <MessagePrimitive.Root
      ...
      data-role="assistant"
      className={cn(
        "fade-in slide-in-from-bottom-1 animate-in relative -mb-7.5 pb-7.5 duration-150 [contain-intrinsic-size:auto_200px]",
        !isLast && "[content-visibility:auto]",
      )}
    >
      <div
